...
function AcademicCard({ post, index }: { post: (typeof academicPosts)[number], index: number }) {
  return (
    <MotionCard index={index} className="w-full h-full">
      <article className="w-full h-full overflow-hidden rounded-2xl border border-[#002a75]/10 bg-white shadow-[0_4px_24px_rgba(0,42,117,0.12)]">
      <div className="relative h-44">
        ...
        <div className="absolute left-3 top-3 rounded-full bg-[#002a75] px-3 py-1 font-[family-name:var(--font-inter)] text-xs font-bold text-white">
          {post.category}
        </div>
        <div
          className={`absolute right-3 top-3 rounded-full border px-3 py-1 font-[family-name:var(--font-inter)] text-xs font-bold ${post.statusClass}`}
        >
          {post.status}
        </div>
      </div>
      <div className="flex min-h-44 flex-col p-5 font-[family-name:var(--font-inter)]">
...
export function AcademicInfoSection() {
  return (
    <section className="relative px-5 py-24 sm:px-8 lg:px-16">
      ...
      />
      <MotionSection className="mx-auto flex max-w-[1120px] flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-4 max-w-2xl font-[family-name:var(--font-inter)] text-sm leading-relaxed text-white/80">
      ...
      <div className="mx-auto mt-12 grid max-w-[1120px] gap-6 md:grid-cols-3">
        {academicPosts.map((post, i) => (
          <AcademicCard post={post} key={post.title} index={i} />
        ))}
      </div>
    </section>
  )
